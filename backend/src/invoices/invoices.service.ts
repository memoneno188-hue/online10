...
export class InvoicesService {
    ...
    private async generateInvoiceCode(type: InvoiceType, date: Date): Promise<string> {
        ...
        if (lastInvoice && lastInvoice.code) {
            // Extract sequence number from code (format: XX-YY-NNNN)
        ...
        let code = `${typePrefix}-${year}-${sequence.toString().padStart(4, '0')}`;
        let attempts = 0;
        ...
        while (attempts < maxAttempts) {
            ...
            sequence++;
            code = `${typePrefix}-${year}-${sequence.toString().padStart(4, '0')}`;
            attempts++;
        }
    ...
    async create(createInvoiceDto: CreateInvoiceDto) {
        ...
        // Check customs number uniqueness for the year (if provided)
        if (invoiceData.customsNo && type !== 'FREE') {
            ...
            if (existing) {
                throw new ConflictException('رقم البيان مُسجَّل مسبقاً لهذه السنة');
            }
        }
